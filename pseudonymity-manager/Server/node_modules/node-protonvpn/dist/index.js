...
function getOpenVpnBin() {
    switch (process.platform) {
        case "win32":
            if (require$$1.existsSync("C:\\Program Files\\OpenVPN\\bin\\openvpn.exe")) {
                return "C:\\Program Files\\OpenVPN\\bin\\openvpn.exe";
            }
            else if (require$$1.existsSync("C:\\Program Files (x86)\\OpenVPN\\bin\\openvpn.exe")) {
...
class OpenVpnController {
    ...
    connect(configPath, auth) {
        ...
        if (require$$1.lstatSync(configPath).isDirectory())
        ...
        this.openVpnProcess = require$$0.spawn(this.openVpnBin, ["--config", configPath]);
...
class ProtonVpnController {
    ...
    validateConfigDir(dir) {
        ...
        if (!require$$1.lstatSync(dir).isDirectory())
            throw new Error("Config directory must be a directory.");
        const files = require$$1.readdirSync(dir);
        if (files.length === 0)
    ...
    getCountryConfigs(dir) {
        ...
        countryCodes.forEach((countryCode) => {
            ...
            configs.set(countryCode, require$$2.resolve(dir, config));
        });
        return configs;
    }
}
...
function fixConfigCompressionError(configPath) {
    ...
    if (!configPath.endsWith(".ovpn"))
        throw new Error("Config file must be an .ovpn file.");
    let f = require$$1.readFileSync(configPath, "utf-8");
    ...
    if (!f.includes("block-outside-dns"))
        f = f.replace("fast-io", "fast-io\nblock-outside-dns");
    require$$1.writeFileSync(configPath, f);
}
function fixConfigCompressionErrorDir(dir) {
    ...
    const files = require$$1.readdirSync(dir);
    for (const file of files) {
        fixConfigCompressionError(require$$2.resolve(dir, file));
    }
}

exports.OpenVpnController = OpenVpnController;
exports.ProtonVpnController = ProtonVpnController;
exports.fixConfigCompressionError = fixConfigCompressionError;
exports.fixConfigCompressionErrorDir = fixConfigCompressionErrorDir;
