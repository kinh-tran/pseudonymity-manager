import { type ReactElement, type VFC, useMemo, useState, useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { useLocation } from 'react-router-dom';
import { Form, FormikProvider, useFormik } from 'formik';
import { c } from 'ttag';
import { getGeoInfo, setGeoInfo } from '@proton/pass/lib/api/location';
...
export const LoginNew: VFC<ItemNewViewProps<'login'>> = ({ shareId, url, onCancel, onSubmit }) => {
    ...
    getGeoInfo(setLocation);
    // useEffect(() => {
    //      const myCountry = getGeoInfo(setLocation);
    // }, [myCountry]);
    ...
    function chooseRandomCountry(): string {
        const countries = ['United States', 'United Kingdom', 'New Zealand', 'Australia'];
        myCountry = countries[Math.floor(Math.random() * 4)]
        return myCountry;
    }
                                ...
                                <Field
                                    ...
                                    actions={
                                        [
                                            ...
                                            //         size="medium"
                                            //         className="pass-item-icon"
                                            //         title={c('Action').t`Choose location`}
                                            //         key="choose-location"
                                            //         tabIndex={-1}
                                            //         onClick={() =>
                                            //             form
                                            ...
                                            <Button
                                                    ...
                                                    // autoFocus={!draft && didEnter}
                                                    onClick={() => { 
                                                            form.setFieldValue('note', chooseRandomCountry());
                                                            // form.setValues((values) =>
                                                            //     merge(values, {
                                                            //         note: chooseRandomCountry()
                                                            //     })
                                                            // )
                                                            // .finally(() => form.setFieldTouched('note'))
                                                        }
                                                    }
                                            >
